...
export default function loaded() {
    ...
    function touchExpand() {
        ...
        if (windowWidth() <= 874) {
            $('.navPages-item > a > .navPages-action-moreIcon').on('click', function mobileExpand(event) {
    ...
    function toggleHeaderHeightDesktop() {
        ...
        if (!$('.header').hasClass('slim')) {
            ...
            // $('.logo-and-toggle').css('width', `${logoWidth}`);
            // $('.navPages-item:first-child .column-menu.navPage-subMenu').css('top', `${headerHeight}px`);
        }

        function compressHeaderDesktop() {
            $('.header').addClass('slim');
            $('.navPages-container').css('top', `-${navHeight + 50}px`);
            // $('.header').css('height', `${navHeight}px`);
            // $('.desktopMenu-toggle').css('left', '0');
            // $('.desktopMenu-toggle').css('width', `${navToggleWidth}px`);
            // $('.logo-and-toggle').css('width', `${logoWidth + navToggleWidth}`);
            // $('body').css('padding-top', `${navHeight}px`);
    ...
    menuColumns.each(function splitColumns() {
        ...
        $lis.each(function buildColumn() {
            ...
            // AH EDIT
            if ($liClass === 'navPage-subMenu-item navPage-straight') {
            ...
            // AH EDIT - START NEW COLUMN EVERY CATEGORY
    ...
    function prepareMenu() {
        if (windowWidth() > 874) {
            ...
            menuColumns.each(function countCols(index) {
    ...
    const el2 = $('.navPages-mega .navPages-item .navPages-action.has-subMenu');
    ...
    for (let i = 0; i < el.length; i++) {
        ...
        hoverintent(
            ...
            function hoverIntentOver() {
                $(this).addClass('hover').unbind('focusin');
    ...
    for (let j = 0; j < el2.length; j++) {
        $(el2[j]).bind('focusin', () => {
            $(el2).each((index, el2el) => {
                $(el2el).parent().removeClass('hover');
                // console.log(el2el);
            });
            $(el2[j]).parent().addClass('hover');
        });

        // console.log($(el2[j]).next().children().last());

        if ($(el2[j]).next().children().length === 1) {
            $(el2[j]).next().children().children().last().bind('focusout', () => {
                $(el2[j]).parent().removeClass('hover');
            });
        } else {
            $(el2[j]).next().children().last().children().last().bind('focusout', () => {
                $(el2[j]).parent().removeClass('hover');
            });
        }
    }
