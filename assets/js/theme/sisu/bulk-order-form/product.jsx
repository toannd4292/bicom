...
const Product = (props) => {
    const {
        name, product_id, image, quantity, updateQuantity, color, size, inStock, stockMessage,
    } = props;
    ...
    return (
        <div className='bulk-form-row'>
            ...
            <div className='bulk-form-col'>{name} <span style={{ color: 'red' }}>{stockMessage}</span>{'\n'}Color: {color}{'\n'}Size: {size}</div>
            <div className='bulk-form-col'>
                <div className='form-increment'>
                    <button className='button button - icon' disabled={!inStock} onClick={() => updateQuantity(quantity - 1, product_id)}>
                    ...
                    <input type='text' className='form-input form-input - incrementTotal' disabled={!inStock} value={quantity} onChange={(e) => { updateQuantity(e.target.value, product_id); }}></input>
                    <button className='button button - icon' disabled={!inStock} onClick={() => updateQuantity(quantity + 1, product_id)}>
